  ...
  <el-card class="bod-card">
    ...
    <el-dialog title="查看项目" :visible.sync="dialogFormVisibleEdit">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="editProject()">承 包</el-button>
      </div>
    </el-dialog>
  </el-card>
</template>
...
export default {
  ...
  methods: {
    ...
    showSetProjectDia(projectId) {
      this.$confirm("此操作将发起项目验收, 是否继续?", "提示", {
        confirmButtonText: "确定发起验收",
        cancelButtonText: "取消",
        type: "warning"
      })
        .then(async () => {
          const res = await this.$http.put(`projects/unacceptance/${projectId}`);
          console.log(res);
